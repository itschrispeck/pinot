...
public class TimeSeriesTransformer implements RecordTransformer {
  ...
  public GenericRow transform(GenericRow record) {
    Object tagsJson = record.getValue(_transformerConfig.getTagsJsonField());
    ...
    if (tagsJson instanceof String) {
      try {
        JsonNode tagsJsonRoot = JsonUtils.stringToJsonNode((String) tagsJson);
    ...
    } else if (tagsJson instanceof Map) {
      // Pre-decoded by kafka decoder
      ((Map<?, ?>) tagsJson).forEach((key, value1) -> tagSet.add(key + "=" + value1));
    }
